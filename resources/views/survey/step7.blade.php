{{-- resources/views/survey/step7.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="utf-8">
    <title>Survei Kepuasan Masyarakat – Langkah 7</title>
        ...
        <div class="max-w-5xl mx-auto mt-8 mb-12 px-4">
                ...
                <div class="w-full max-w-3xl">
                    <div class="h-2 rounded-full bg-slate-200 overflow-hidden">
                        <div
                            ...
                            style="width: {{ $progressPercent ?? 41 }}%;"
                        ></div>
                    </div>
                </div>
            </div>
            ...
            <section class="mt-6 flex justify-center">
                <div class="w-full max-w-4xl bg-[#E8F0FB] rounded-[10px] border border-[#D0D9EE] px-8 py-10">
                    ...
                    <h2 class="text-center text-[16px] md:text-[18px] font-semibold text-slate-900 mb-2">
                        Silahkan Pilih Kelurahan
                    </h2>
                    <p class="text-center text-[12px] md:text-[13px] text-slate-700 mb-6">
                        Kecamatan: <span class="font-semibold">{{ $kecamatan }}</span>
                    </p>
                    ...
                    @php
                        $selected = old('kelurahan', session('survey.kelurahan'));
                    @endphp

                    <form method="POST" action="{{ route('survey.step7.post') }}">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            {{-- Baris bawah (hanya kalau ada data) --}}
                            @if (!empty($row2))
                                <div class="flex items-center justify-center gap-4 md:gap-6 flex-wrap">
                                    @foreach ($row2 as $opt)
                                        <label class="cursor-pointer">
