...
<head>
    ...
    class="inline-block px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 font-semibold">
    ← Kembali ke Arsip
    </a>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($laporan->detail_gender ?? [] as $g => $count)
                <tr>
                    <td>{{ $g }}</td>
                    <td class="text-center">{{ $count }}</td>
                    <td class="text-center">
                        {{ $laporan->total_responden > 0 
                            ? number_format(($count / $laporan->total_responden) * 100, 2, ',', '.') 
                            : '0,00' }}
                    </td>
                </tr>
            @endforeach
            <tr class="total-row">
                <td>Total</td>
                <td class="text-center">{{ $laporan->total_responden }}</td>
                <td class="text-center">{{ $laporan->total_responden > 0 ? '100' : '0' }}</td>
            </tr>

            {{-- Kelompok Umur --}}
            <tr class="section-title"><td colspan="3">Kelompok Umur</td></tr>
            @foreach($laporan->detail_usia ?? [] as $u => $count)
                <tr>
                    <td>{{ $u }}</td>
                    <td class="text-center">{{ $count }}</td>
                    <td class="text-center">
            ...
            @foreach($laporan->detail_pendidikan ?? [] as $p => $count)
                <tr>
                    <td>{{ $p }}</td>
                    <td class="text-center">{{ $count }}</td>
                    <td class="text-center">
            ...
            @foreach($laporan->detail_pekerjaan ?? [] as $pk => $count)
                <tr>
                    <td>{{ $pk }}</td>
                    <td class="text-center">{{ $count }}</td>
                    <td class="text-center">
    ...
    @php
        ...
        $nilaiIndeks = collect($laporan->detail_unsur ?? [])->avg();
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($urut as $kode)
                @php
                    $nrr = $laporan->detail_unsur[$kode] ?? 0;
                    $tertimbang = $nrr / 9;
                @endphp
                <tr>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>{{ $laporan->opd_kode }}</td>
                <td class="text-center">{{ number_format($nilaiIndeks, 2, ',', '.') }}</td>
                <td class="text-center">{{ number_format($ikm, 2, ',', '.') }}</td>
                <td class="text-center">{{ $laporan->mutu }}</td>
                <td class="text-center">
                    @php
                        $kepuasan = 'Tidak Baik';
